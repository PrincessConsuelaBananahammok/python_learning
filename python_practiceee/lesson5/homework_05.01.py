...
for car, datum in car_data.items():
    color, year, engine_volume, car_type, price = datum
    if year >= search_year and engine_volume >= search_engine_volume and price <= search_price:
        new_car_list[car] = datum


def filter_by(item):
    value = item[1]
    *_, filter_price = value
    return filter_price


sorted_cars = sorted(new_car_list.items(), key=filter_by)


for car, datum in sorted_cars[:5]:
    print(f"Model {car} - characteristics:", *datum)
